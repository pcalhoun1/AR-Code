...
for (sim in 1:nsim) {
  rf_ailerons<-growRF_Parallel(ntrees=ntrees, formula=form, data=ailerons, search="exhaustive", method="anova", split="MSE",
                               mtry=13, nsplit=NULL, minsplit=6, minbucket=3, maxdepth=30, sampleMethod='bootstrap', useRpart=TRUE, iseed=sim)
  sss_ailerons<-growRF_Parallel(ntrees=ntrees, formula=form, data=ailerons, search="sss", method="anova", split="MSE",
                                mtry=13, nsplit=NULL, minsplit=6, minbucket=3, maxdepth=30, a=50, sampleMethod='bootstrap', iseed=sim)
  er_ailerons<-growRF_Parallel(ntrees=ntrees, formula=form, data=ailerons, search="exhaustive", method="anova", split="MSE",
                               mtry=13, nsplit=1, minsplit=6, minbucket=3, maxdepth=30, sampleMethod='bootstrap', iseed=sim)
...
#write.table(outData, file = "Results/predAilerons.csv", sep = ",", row.names=FALSE)
