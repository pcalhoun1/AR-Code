...
for (sim in 1:nsim) {
  rf_airquality<-growRF_Parallel(ntrees=ntrees, formula=form, data=airquality, search="exhaustive", method="anova", split="MSE",
                                 mtry=2, nsplit=NULL, minsplit=6, minbucket=3, maxdepth=30, sampleMethod='subsample', useRpart=TRUE, iseed=sim)
  sss_airquality<-growRF_Parallel(ntrees=ntrees, formula=form, data=airquality, search="sss", method="anova", split="MSE",
                                  mtry=2, nsplit=NULL, minsplit=6, minbucket=3, maxdepth=30, a=50, sampleMethod='subsample', iseed=sim)
  er_airquality<-growRF_Parallel(ntrees=ntrees, formula=form, data=airquality, search="exhaustive", method="anova", split="MSE",
                                 mtry=2, nsplit=1, minsplit=6, minbucket=3, maxdepth=30, sampleMethod='subsample', iseed=sim)
...
write.table(outData, file = "predAirquality_subsample.csv", sep = ",", row.names=FALSE)
