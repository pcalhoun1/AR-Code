...
for (sim in 1:nsim) {
  rf_friedman2<-growRF_Parallel(ntrees=ntrees, formula=form, data=friedman2, search="exhaustive", method="anova", split="MSE",
                                mtry=1, nsplit=NULL, minsplit=6, minbucket=3, maxdepth=30, sampleMethod='subsample', useRpart=TRUE, iseed=sim)
  sss_friedman2<-growRF_Parallel(ntrees=ntrees, formula=form, data=friedman2, search="sss", method="anova", split="MSE",
                                 mtry=1, nsplit=NULL, minsplit=6, minbucket=3, maxdepth=30, a=50, sampleMethod='subsample', iseed=sim)
  er_friedman2<-growRF_Parallel(ntrees=ntrees, formula=form, data=friedman2, search="exhaustive", method="anova", split="MSE",
                                mtry=1, nsplit=1, minsplit=6, minbucket=3, maxdepth=30, sampleMethod='subsample', iseed=sim)
...
write.table(outData, file = "predFriedman2_subsample.csv", sep = ",", row.names=FALSE)
